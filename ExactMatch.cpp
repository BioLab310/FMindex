...
uint32_t* calc_SArangeSeq(sFMindex *pmem,char *read)
{
	...
	ch = read[i];
	p[0] = calc_C(*pmem,ch) + 1;
	p[1] = calc_C(*pmem,C_next(ch)) + 1;
	i--;
	while(p[0] <= p[1] && i >= 0)
	{
		ch = read[i];
		p[0] = LF_Mapping_l(*pmem,ch,p[0]);
		p[1] = LF_Mapping_h(*pmem,ch,p[1]);
		i--;
	}
	return p;
}
...
{
	uint32_t *p = (uint32_t *)malloc(sizeof(uint32_t)*2);
	p[0] = LF_Mapping_l(*pmem,ch,pre[0]);
	p[1] = LF_Mapping_h(*pmem,ch,pre[1]);
	return p;
}
